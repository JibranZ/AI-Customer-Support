...
export async function POST(req) {
  try {
    const { messageID, feedback, chatMessage, userMessage, detailedFeedback } = await req.json();

    if (!messageID || !feedback || !['thumbsUp', 'thumbsDown', 'detailed'].includes(feedback)) {
      return NextResponse.json({ error: "Invalid input" }, { status: 400 });
    }
    ...
    const newFeedback = new Feedback({
      ...
      chatMessage,
      userMessage,
      detailedFeedback: feedback === 'thumbsDown' ? detailedFeedback : undefined,
    });
